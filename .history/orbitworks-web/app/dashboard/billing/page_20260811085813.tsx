...
export default function BillingPage() {
  ...
  async function handleCheckoutSuccess() {
    setClientSecret(null);
    setUpgradingTierKey(null);
    // The webhook updates Firestore asynchronously - give it a moment,
    // then refresh what we show. If it hasn't landed yet, a manual
    // refresh of this page shortly after will pick it up.
    setTimeout(loadCompany, 2000);
  }

  const trialEndsAtDate = company?.trialEndsAt ? company.trialEndsAt.toDate() : null;
  const isTrialing = company?.subscriptionStatus === "trialing";
  const daysLeftInTrial =
    isTrialing && trialEndsAtDate
            ...
            {isTrialing && daysLeftInTrial !== null && (
              <p className="mt-3 rounded-lg bg-accent/10 px-3.5 py-2 text-xs font-medium text-accent">
                {daysLeftInTrial > 0
                  ...
                  : "Your trial has ended. Choose a plan below to keep using Orbitsworks."}
              </p>
            )}
          </div>
