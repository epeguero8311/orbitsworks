...
export default function ReportsPage() {
  ...
  async function runReport() {
    ...
    try {
      ...
      // Fetch employees so we can attach hourly rates for the payroll estimate
  ...
  const totalEstimatedPay =
    summaries?.reduce((sum, s) => sum + (s.estimatedPay ?? 0), 0) ?? 0;
  const anyRatesMissing = summaries?.some((s) => s.hourlyRate == null) ?? false;
            ...
            <div className="rounded-lg border border-gray-200 bg-white p-4">
              ...
              {anyRatesMissing && (
                <p className="mt-1 text-xs text-amber-600">
                  Some employees are missing an hourly rate — add one on the
                  Employees page for a complete estimate.
                </p>
              )}
            </div>
            <div className="rounded-lg border border-gray-200 bg-white p-4">
